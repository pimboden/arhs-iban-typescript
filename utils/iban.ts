...
*/
import { Specification } from "~/models/iban"

let A = 'A'.charCodeAt(0),
  Z = 'Z'.charCodeAt(0),
  NON_ALPHANUM = /[^a-zA-Z0-9]/g,
  EVERY_FOUR_CHARS = /(.{4})(?!$)/g

  var countries : { [key: string]: Specification } = {}
/**
...
const parseStructure = (structure: string): RegExp => {
  ...
  let matches = structure.match(/(.{3})/g)
  if (matches == null) throw "structure doesn't match"

  var regex = matches.map((block) => {
...
const electronicFormat = (iban: string): string =>
  iban.replace(NON_ALPHANUM, '').toUpperCase()
...
const addSpecification =(iban: Specification) =>{
  countries[iban.countryCode] = iban
     ...
     */
const isString = (v: any): boolean=>(typeof v == 'string' || v instanceof String);
...
const isValid = (iban:string): boolean =>{
  ...
  let countryStructure = countries[iban.slice(0,2)];
  return !!countryStructure && countryStructure.isValid(iban);
     ...
     * @returns {string|*}
     */
     const toBBAN = (iban:string , separator:string): string | any => {
 ...
 */
const isValidBBAN = (countryCode:string , bban:string) => {
    if (!isString(bban)){
        return false;
    }
    var countryStructure = countries[countryCode];
 ...
 * @returns {string}
 */
const printFormat = (iban:string, separator:string | undefined ): string => {
...
fillData()
export {iso13616Prepare, iso7064Mod97_10, parseStructure, isValid,isValidBBAN, toBBAN, fromBBAN, printFormat, countries}
